...
function TrackScreen({onBack, lastOrderId, isHi, t, fam}) {
  ...
  useEffect(()=>{
    ...
    const {doc:fdoc}=require('firebase/firestore');
    const unsub = onSnapshot(
      fdoc(db,'orders',lastOrderId),
...
function RiderApp({rider,data,setData,onBack}) {
  ...
  const accept=async id=>{
    const ord=riderOrds.find(o=>o.id===id)||realOrders?.find(o=>o.id===id);
    if(ord&&!['packed','ready','confirmed'].includes(ord.status)){
      alert('⚠️ Order abhi ready nahi hai! Shop se pack hone ka wait karo.');
      return;
    }
    try{
      const {doc,updateDoc}=await import('firebase/firestore');
      await updateDoc(doc(db,'orders',id),{riderId:rider.id,status:'out'});
    }catch(e){console.log(e);}
  };
...
function AdminApp({data,setData,onBack}) {
  ...
  const toggleProd=async(id)=>{
    ...
    try{
      const {doc:fdoc,updateDoc,addDoc:fadd}=await import('firebase/firestore');
      if(prod.firestoreId){
        await updateDoc(fdoc(db,'products',prod.firestoreId),{active:newActive});
      }
  ...
  const saveProd=async(id,ch)=>{
    ...
    try{
      ...
      if(prod?.firestoreId){
        await updateDoc(fdoc(db,'products',prod.firestoreId),ch);
      }
  ...
  const addNewProd=async()=>{
    ...
    try{
      const {addDoc:fadd,collection:fcol}=await import('firebase/firestore');
      const ref=await fadd(fcol(db,'products'),p);
  ...
  const deleteProd=async(id)=>{
    ...
    try{
      if(prod?.firestoreId){
        ...
        await deleteDoc(fdoc(db,'products',prod.firestoreId));
      }
            ...
            : realOrders.length===0
              ...
              : realOrders.map((o,i)=>(
                <div key={o.id} className="gc" style={{padding:14,marginBottom:10,animation:`fadeUp .3s ease ${i*.04}s both`}}>
                  ...
                  {o.status!=='delivered'&&<div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                    {[
                      {s:'pending',   l:'🕐 Pending'},
                      {s:'confirmed', l:'✅ Confirm'},
                      {s:'packed',    l:'📦 Packed'},
                      {s:'out',       l:'🚴 Out'},
                      ...
                      <button key={st.s} onClick={async()=>{
                        try{
                          ...
                          await updateDoc(fd(db,'orders',o.id),{status:st.s});
                        }catch(e){console.log(e);}
                      }} style={{padding:'5px 10px',fontSize:11,fontWeight:700,borderRadius:50,background:'rgba(61,255,122,.08)',border:'1px solid rgba(61,255,122,.2)',color:'#3DFF7A',cursor:'pointer',fontFamily:"'Outfit',sans-serif"}}>
                        {st.l}
                      </button>
                    ))}
                  </div>}
