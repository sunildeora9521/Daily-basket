...
const [adIdx, setAdIdx]=useState(0);
  ...
  // Sync products from Firestore live
  useEffect(()=>{
...
function AdminApp({data,setData,onBack}) {
  ...
  useEffect(()=>{
    const unsub4=onSnapshot(collection(db,'products'),snap=>{
      if(snap.docs.length>0){
        const fsProds=snap.docs.map(d=>({...d.data(),firestoreId:d.id}));
        setData(d=>({...d,products:fsProds}));
      }
    });
    return()=>unsub4();
  ...
  const addNewProd=async()=>{
    ...
    const p={id:Date.now(),name:npF.name,nameHi:npF.nameHi||npF.name,price:+npF.price,unit:npF.unit,emoji:npF.emoji||'🥦',imgUrl:npF.imgUrl||'',cat:npF.cat,stock:+npF.stock,tag:'New',tagHi:'नया',active:true,createdAt:new Date().toISOString()};
    try{
