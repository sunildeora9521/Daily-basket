...
useEffect(()=>{
  getDocs(collection(db,'adSlides')).then(snap=>{
    if(snap.docs.length>0){
      const fs=snap.docs.map(d=>({...d.data(),firestoreId:d.id})).slice(0,6);
      setAdSlides(fs);
    }
  }).catch(()=>{});
},[]);
...
export default function DailyBasket() {
 ...
 const handleCapSelect = id => {
  ...
  let content;
  try { content=renderContent(); } catch(e) {
    console.error('Render error:',e);
    content=<div style={{color:'#3DFF7A',padding:20,fontSize:14}}>Loading... please wait</div>;
  }
  return (
