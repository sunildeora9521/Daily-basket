  ...
  {ckStep===3&&<>
    ...
    if(scr==='home') return (
      <div style={{paddingBottom:168,fontFamily:fam}}>
        ...
        {adSlides.length>0&&<div style={{margin:'0 0 16px',position:'relative',overflow:'hidden'}}>
          <div onClick={()=>{try{const sl=adSlides[safeAdIdx];if(!sl)return;if(sl.link==='milk')setScrMilk(true);else if(sl.link==='bulk')setScrBulk(true);else if(sl.link)window.open(sl.link,'_blank');}catch(e){}}} style={{background:(adSlides[safeAdIdx]||{}).bg||'#0D2010',padding:'22px 20px 20px',position:'relative',overflow:'hidden',minHeight:180,cursor:'pointer',transition:'background .5s ease'}}>
            {/* Big emoji background */}
            {(adSlides[safeAdIdx]||{}).imgUrl
            ? <img src={(adSlides[safeAdIdx]||{}).imgUrl} style={{position:'absolute',right:0,top:0,height:'100%',width:'45%',objectFit:'cover',opacity:.4}}/>
            : <div style={{position:'absolute',right:-20,top:'50%',transform:'translateY(-50%)',fontSize:130,opacity:.18,transition:'all .5s',filter:'blur(2px)'}}>{(adSlides[safeAdIdx]||{}).emoji||'🛒'}</div>
          }
            {/* Decorative circles */}
            ...
            <div style={{position:'relative',zIndex:1}}>
              <div style={{display:'inline-flex',alignItems:'center',gap:6,background:'rgba(255,255,255,.12)',backdropFilter:'blur(10px)',borderRadius:50,padding:'4px 12px',marginBottom:12}}>
                <span style={{fontSize:11,fontWeight:700,color:'#fff',letterSpacing:.5}}>{(adSlides[safeAdIdx]||{}).chip||''}</span>
              </div>
              <div style={{fontSize:26,fontWeight:900,lineHeight:1.2,maxWidth:220,fontFamily:fam,color:'#fff',textShadow:'0 2px 12px rgba(0,0,0,.4)'}}>{(adSlides[safeAdIdx]||{}).title||''}</div>
              <div style={{fontSize:13,color:'rgba(255,255,255,.65)',marginTop:6,fontWeight:500}}>{(adSlides[safeAdIdx]||{}).sub||''}</div>
              <button className="btn rip" onClick={e=>{e.stopPropagation();const sl=adSlides[safeAdIdx];if(!sl)return;if(sl.link==='milk')setScrMilk(true);else if(sl.link==='bulk')setScrBulk(true);else if(sl.link)window.open(sl.link,'_blank');else if(sl.act)sl.act();}} style={{marginTop:14,padding:'10px 24px',fontSize:13,fontFamily:fam,fontWeight:800}}>{(adSlides[safeAdIdx]||{}).btn||'Learn More'} →</button>
            </div>
          </div>
          {/* Dots */}
          <div style={{display:'flex',justifyContent:'center',gap:5,marginTop:10}}>
            {adSlides.map((_,i)=><div key={i} onClick={()=>setAdIdx(i)} style={{width:i===safeAdIdx?24:6,height:6,borderRadius:3,background:i===safeAdIdx?'#3DFF7A':'rgba(61,255,122,.25)',cursor:'pointer',transition:'all .3s'}}/>)}
          </div>
        </div>}
        {/* Search Bar */}
  ...
  return (
    <div style={{paddingBottom:100,fontFamily:fam}}>
      ...
      <div style={{padding:'0 20px',display:'flex',flexDirection:'column',gap:8}}>
        ...
        <button className="btng rip" onClick={()=>{if(window.confirm('Sign out karna chahte ho?')){import('firebase/auth').then(({signOut,getAuth})=>{signOut(getAuth()).then(()=>window.location.reload());});}}} style={{width:'100%',padding:'14px',fontSize:13,color:'#FF6B6B',border:'1px solid rgba(255,107,107,.2)',marginTop:4,fontFamily:fam}}>{t.signOut}</button>
      </div>
    </div>
  );
}
            ...
            {active.map(o=>(
              <div key={o.id} style={{marginBottom:8}}>
                ...
                <div style={{display:'flex',gap:8,alignItems:'center'}}>
                  ...
                  <button onClick={()=>window.open(`https://maps.google.com/?q=${encodeURIComponent(o.address)}`,'_blank')} style={{padding:'5px 12px',borderRadius:50,background:'rgba(61,255,122,.1)',border:'1px solid rgba(61,255,122,.3)',color:'#3DFF7A',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:"'Outfit',sans-serif"}}>🗺️ Navigate</button>
...
function AdminApp({data,setData,onBack}) {
  ...
  // Products synced via customer app listener (shared data state)
  // No separate admin listener needed - avoids duplicates
  ...
  const addNewProd=async()=>{
    ...
    try{
      ...
      alert('Firestore save failed, local mein add hua: '+e.message);
    }
      ...
      <div style={{display:'flex',gap:8}}>
        ...
        {tab==='slides'&&<>
          <div className="sh">
            ...
            <button className="btn rip" onClick={()=>setAddSlide(true)} style={{padding:'7px 14px',fontSize:12}}>+ Add Slide</button>
          </div>
          ...
          {editSlide&&<AdminModal onClose={()=>setEditSlide(null)}>
            ...
            {[{l:'Emoji',k:'emoji'},{l:'Badge Text',k:'chip'},{l:'Title',k:'title'},{l:'Subtitle',k:'sub'},{l:'Button Text',k:'btn'},{l:'Link',k:'link'}].map(f=>(
              <div key={f.k} style={{marginBottom:10}}>
                <div style={{fontSize:10,color:'var(--t3)',marginBottom:4}}>{f.l.toUpperCase()}</div>
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(e,info){
    console.error('App crash:',e.message,info);
  ...
  render(){
    if(this.state.hasError){
      return <div style={{position:'fixed',inset:0,background:'#060906',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:24}}>
        <div style={{fontSize:48,marginBottom:16,animation:'statusP 1s infinite'}}>🧺</div>
        <div style={{color:'#3DFF7A',fontSize:18,fontWeight:800,marginBottom:8}}>Daily Basket</div>
        <div style={{color:'#8A9A8A',fontSize:13,marginBottom:8,textAlign:'center'}}>Loading...</div>
        <div style={{color:'#5A6A5A',fontSize:10,marginBottom:24}}>Auto-retrying...</div>
        <button onClick={()=>{this.setState({hasError:false});window.location.reload();}} style={{padding:'12px 32px',background:'linear-gradient(135deg,#3DFF7A,#00C44F)',color:'#0A1A0A',border:'none',borderRadius:12,fontSize:14,fontWeight:800,cursor:'pointer'}}>🔄 Refresh App</button>
      </div>;
    }
