  ...
  const getGPS=()=>{
    ...
    navigator.geolocation.getCurrentPosition(async pos=>{
      try{
        ...
        const res=await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${latitude}&lon=${longitude}&format=json&zoom=18&addressdetails=1`);
        ...
        setFlat([house,road].filter(Boolean).join(', ')||suburb||'');
        setArea(suburb||village||road||'');
        setCity(city||village||'Bhopalgarh');
      }catch(e){alert('Location fetch failed: '+e.message);}
      setGpsLoading(false);
    },err=>{
      const msgs={1:'GPS permission denied. Settings mein allow karo.',2:'Location unavailable.',3:'Timeout. Phir try karo.'};
  ...
  try{
    ...
    if(auth.currentUser?.uid){
      import('firebase/firestore').then(({doc,setDoc})=>{
        setDoc(doc(db,'users',auth.currentUser.uid),{points:newPoints},{merge:true}).catch(()=>{});
      });
    }
    setUsePoints(false);
