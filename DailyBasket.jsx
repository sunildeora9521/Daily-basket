  ...
  const getGPS=()=>{
    ...
    navigator.geolocation.getCurrentPosition(async pos=>{
      try{
        ...
        // Store raw coords for precise delivery
        ...
        // Build flat: house + road
        const flatStr=[house,road].filter(Boolean).join(' ')||hamlet||suburb||'Near '+town;
        // Area: most specific locality
        const areaStr=village||suburb||hamlet||road||town;
        // City: actual city/town (not tehsil/district)
        const cityStr=town||village||district||'Bhopalgarh';
        setFlat(flatStr);
        setArea(areaStr);
        setCity(cityStr);
        // Show accuracy info
        if(accuracy>100) alert(`📍 Location mili! (±${Math.round(accuracy)}m accuracy)
Sahi lagti hai to address confirm karo, warna manually edit karo.`);
      }catch(e){alert('Location fetch failed. Manual address likho.');}
      setGpsLoading(false);
    },err=>{
...
try { localImg=localStorage.getItem('prodImg_'+d.id); } catch(e) {}
  ...
  if(!finalAddr){alert('⚠️ Address set karo pehle!');return;}
