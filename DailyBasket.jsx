  ...
  useEffect(()=>{
    const unsub=onSnapshot(collection(db,'products'),snap=>{
      const fsProds=snap.docs
        .map(d=>({...d.data(),id:d.data().id||d.id,firestoreId:d.id}))
        .filter(p=>p.name&&p.active!==false); // only valid active products
      if(fsProds.length>0){
        setData(d=>({...d,products:fsProds}));
      }
      // If 0 products in Firestore, keep default hardcoded products
    });
    return()=>unsub();
  },[]);
        ...
        <div style={{margin:'0 0 16px',position:'relative',overflow:'hidden'}}>
          <div onClick={adSlides[adIdx].act} style={{background:adSlides[adIdx].bg,padding:'22px 20px 20px',position:'relative',overflow:'hidden',minHeight:180,cursor:'pointer',transition:'background .5s ease'}}>
            ...
            <div style={{position:'relative',zIndex:1}}>
              ...
              <button className="btn rip" onClick={e=>{e.stopPropagation();const sl=adSlides[adIdx];if(sl.link==='milk')setScrMilk(true);else if(sl.link==='bulk')setScrBulk(true);else if(sl.link)window.open(sl.link,'_blank');}} style={{marginTop:14,padding:'10px 24px',fontSize:13,fontFamily:fam,fontWeight:800}}>{adSlides[adIdx].btn} →</button>
            </div>
          </div>
          {/* Dots */}
