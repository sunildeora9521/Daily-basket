  ...
  useEffect(()=>{
    ...
    const unsub=onSnapshot(collection(db,'notifications'),snap=>{
      snap.docChanges().forEach(ch=>{
        if(ch.type==='added'){
          const n=ch.doc.data();
          if((n.userId===auth.currentUser?.uid||n.broadcast===true)&&!n.read){
            if(Notification.permission==='granted') new Notification(n.title||'Daily Basket 🛒',{body:n.body||'',icon:'/icon-192.png'});
    ...
    if(scr==='home') return (
      <div style={{paddingBottom:168,fontFamily:fam}}>
        ...
        <div style={{margin:'0 0 16px',position:'relative',overflow:'hidden'}}>
          <div onClick={()=>{const sl=adSlides[adIdx];if(!sl)return;if(sl.link==='milk')setScrMilk(true);else if(sl.link==='bulk')setScrBulk(true);else if(sl.link)window.open(sl.link,'_blank');else if(sl.act)sl.act();}} style={{background:(adSlides[adIdx]||{}).bg||'#0D2010',padding:'22px 20px 20px',position:'relative',overflow:'hidden',minHeight:180,cursor:'pointer',transition:'background .5s ease'}}>
            ...
            <div style={{position:'relative',zIndex:1}}>
              ...
              <button className="btn rip" onClick={e=>{e.stopPropagation();const sl=adSlides[adIdx];if(!sl)return;if(sl.link==='milk')setScrMilk(true);else if(sl.link==='bulk')setScrBulk(true);else if(sl.link)window.open(sl.link,'_blank');else if(sl.act)sl.act();}} style={{marginTop:14,padding:'10px 24px',fontSize:13,fontFamily:fam,fontWeight:800}}>{(adSlides[adIdx]||{}).btn||'Learn More'} →</button>
            </div>
          </div>
          {/* Dots */}
...
function AdminApp({data,setData,onBack}) {
  ...
  const [newSlide,setNewSlide]=useState({emoji:'🎯',chip:'New Slide',title:'',sub:'',btn:'Learn More',link:'',bg:'linear-gradient(135deg,#0D2010,#0A180A)'});
      ...
      <div style={{display:'flex',gap:8}}>
        ...
        {tab==='slides'&&<>
          ...
          {addSlide&&<AdminModal onClose={()=>setAddSlide(false)}>
            ...
            {[{l:'Emoji',k:'emoji',ph:'🎯'},{l:'Badge Text',k:'chip',ph:'e.g. 🔥 Hot Deal'},{l:'Title',k:'title',ph:'Main heading'},{l:'Subtitle',k:'sub',ph:'Short description'},{l:'Button Text',k:'btn',ph:'e.g. Shop Now'},{l:'Link (URL / milk / bulk)',k:'link',ph:'URL ya milk ya bulk ya blank'}].map(f=>(
              <div key={f.k} style={{marginBottom:10}}>
                <div style={{fontSize:10,color:'var(--t3)',marginBottom:4}}>{f.l.toUpperCase()}</div>
            ...
            <div style={{display:'flex',gap:8}}>
              <button className="btn rip" onClick={async()=>{
                ...
                setAddSlide(false);
                setNewSlide({emoji:'🎯',chip:'New Slide',title:'',sub:'',btn:'Learn More',link:'',bg:'linear-gradient(135deg,#0D2010,#0A180A)'});
                alert('✅ Slide added!');
